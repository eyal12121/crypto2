...
def main():
    ...
    main_server.connection_loss()  # Simulate connection loss to a single server
    assert (clients[0].request_file(file_path, output_files[5])[0])  # client request file it did not add to system
    main_server.corrupt_data(file_path)  # Simulate corrupted data (malicious server)
    ...
    main_server.connection_loss()  # Simulate connection loss to a single server
    assert (not clients[0].request_file(file_path, output_files[8])[0])  # client request file it did not add to system
